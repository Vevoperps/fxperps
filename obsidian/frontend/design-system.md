...
@layer utilities {   /* single-purpose helpers: .pixelated … */ }
```
...
### What this project added on top

This build is a WebGL scene ([[chess-scene]]), so the theme it grew is small and
dark-only:

| Group | Tier 1 | Tier 2 | Binding |
|-------|--------|--------|---------|
| Brand blue | `--raw-color-brand-500/600/700` | `--background`, `--foreground`, `--backdrop-wave-low/high` | `--color-background`, `--color-foreground`, `--color-backdrop-wave-low/high` |
| Scene frame | `--raw-aspect-scene`, `--raw-size-scene-fill`, `--raw-radius-scene-frame` | `--scene-aspect`, `--scene-frame-width`, `--scene-frame-radius` | `--container-scene-frame` → `max-w-scene-frame`, `--radius-scene-frame` → `rounded-scene-frame` |

Two things worth copying:

- **`--background` is overridden in `:root`, not only in the dark-mode block.**
  There is one colour scheme and no light theme to serve. It was originally the
  same black as the WebGL backdrop so canvas and page shared a colour; the page
  is now brand blue and the card is deliberately a separate surface.
- **`--scene-frame-width` does its `min()` arithmetic in Tier 2**, not in
  `@theme`. Rule 4 forbids `calc()` in a `@theme` entry, and Tier 2 is allowed
  to compose primitives — so the computed value lands there and `@theme` still
  binds a plain `var()`.

The `@layer base/utilities/components` blocks now carry the adaptive grid, the
`.pixelated` helper, and `.custom-cursor-active` (which hides the native cursor
for [[components/common]]'s `Cursor3D` — note the name, since `.cursor-none`
would collide with Tailwind's own utility).

> [!note] Two token groups and a `@layer components` block were deleted with the
> controls panel
> `--raw-color-panel-*` / `--surface-panel*` / `--action-panel`, the
> `--*-control-*` native-control geometry, the `.panel-range` and `.panel-swatch`
> pseudo-element styles, and the `.scrollbar-none` utility all went with it
> (ADR-0023). They were the one place in this project where `@layer components`
> was genuinely unavoidable — range and colour inputs expose their track, thumb
> and swatch only through vendor pseudo-elements. Worth knowing if that ever
> comes back.
...
Two faces, both **local** (`next/font/local`), loaded in `src/app/layout.tsx`
from `src/app/fonts/` and exposed on `<body>`:

| Face | File | Variable | Binding | Used for |
|------|------|----------|---------|----------|
| General Sans Medium | `GeneralSans-Medium.otf` | `--font-general-sans` | `--font-sans` | nav, copy, labels, buttons, logo |
| Jersey 25 | `Jersey25-Regular.ttf` | `--font-jersey` | `--font-display` | headline and the stat figures |

The starter's **Onest** (Google Fonts) was removed — it was a third family doing
General Sans's job, and it kept loading whether or not anything used it.

Type scale, measured off the Figma frame (see [[home-hero]]):

| Token | Size / line-height | Where |
|-------|--------------------|-------|
| `text-display` | 48px / **0.8** | headline, stat figures |
| `text-display-wide` | 96px | the headline only, at `wide:` and above |
| `text-body` | 16 / 19px | everything else |
| `text-logo` | 18 / 22px | the wordmark |

> [!warning] `--breakpoint-wide` is a literal in `@theme`, and has to be
> A media query condition cannot resolve a custom property, so binding it
> through Tier 2 emits `@media (width >= var(--wide))` and fails to parse. The
> same forced exception the starter already makes for `--leading-display` and
> `--ease-entrance`. Its value (1441px) is the first pixel of the adaptive
> grid's next band — see [[home-hero]] for why the headline can only double
> there.
