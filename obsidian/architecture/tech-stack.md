...
| Package | Version | Role |
|---------|---------|------|
| `three` | `0.185.1` | WebGL renderer, loaders, PMREM — drives the home page scene |
| `postprocessing` | `^6.39.4` | Bloom / depth of field / vignette chain |
| `@types/three` | `0.185.1` | Types (dev) |

Used **directly**, not through react-three-fiber — R3F's global JSX augmentation
breaks the vendored spring engine. ADR: [[decisions-log]] ADR-0018. Full docs:
[[chess-scene]].

The Draco decoder is vendored to `public/draco/` (copied from
`three/examples/jsm/libs/draco/gltf/`) so decoding never costs a CDN round-trip
on the critical path. Re-copy it when `three` is upgraded.
