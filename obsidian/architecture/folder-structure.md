...
│   ├── fonts/              # Local font files loaded by next/font/local
│   ├── icon.tsx · apple-icon.tsx · opengraph-image.tsx
│   │                       # Brand mark drawn at build time — see [[seo-metadata]]
...
│   └── home/               # HomeView + its hero components — see [[home-hero]]
│       ├── index.tsx       # the view: shell, three columns, bot gate
│       ├── site-header.tsx · hero-intro.tsx · stat-grid.tsx
│       ├── headline.tsx    # the headline, set letter by letter
│       ├── reveal.tsx      # one staged entrance, gated on the load curtain
│
├── data/mocks/             # Placeholder content, passed into views by props
│   └── home.ts
│
...
│   ├── common/             # Shared infrastructure (chess-scene, cursor-3d,
│   │                       #   pixel-waves, preloader, Cookie, grid,
│   │                       #   ReducedMotion)
...
│   ├── backdrop-settings.ts # Pixel-wave background store — see [[components/common]]
│   ├── css-color.ts        # Resolve a CSS custom property to RGB (canvas painting)
│   ├── cursor-settings.ts  # Hover-cursor angles — see [[components/common]]
│   ├── preloader.ts        # Load-curtain state — scene-ready gate
│   ├── cursor-scene.ts     # The 3D cursor's own tiny WebGL context
│   ├── pointer.ts          # App-wide pointer singleton, reference-counted
│   ├── scene/              # The WebGL chess scene — see [[chess-scene]]
...
```
public/
├── manifest.json            # the only meta asset left at the root; the icons and
│                            #   share card are generated — see [[seo-metadata]]
